...
class IPAddressValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, ip_address: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in IP address input
        ...
        malicious_patterns = [
            ...
            # Non-IP characters (except expected IPv4/IPv6)
            r'[^0-9a-fA-F:\.]',
        ]
        
        ip_check = ip_address.lower()
        for pattern in malicious_patterns:
            if re.search(pattern, ip_check, re.IGNORECASE):
                return True
    ...
    def validate(self, 
        ...
        try:
            ...
            MAX_SECURE_LENGTH = 45  # IPv6 max length is 39, add buffer
            if len(ip_address) > MAX_SECURE_LENGTH:
                return ValidationResult(
                    is_valid=False,
                    id_type=IDType.IP_ADDRESS,
                    confidence=0.0,
                    errors=[f'IP address too long (max {MAX_SECURE_LENGTH} characters)'],
                    metadata={}
                )
            
            # Security: Check for malicious patterns
            if self._contains_malicious_patterns(ip_address):
                return ValidationResult(
                    is_valid=False,
                    id_type=IDType.IP_ADDRESS,
                    confidence=0.0,
                    errors=['IP address contains potentially malicious content'],
                    metadata={}
                )
