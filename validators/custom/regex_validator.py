...
class CustomRegexValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, value: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in regex input
        ...
        malicious_patterns = [
            ...
            # Regex bomb patterns (simplified detection)
            r'(\*\+|\+\*|\?\+|\+\?|\{\d+,\}.*\{\d+,\})',
        ]
        
        value_check = value.lower()
        for pattern in malicious_patterns:
            if re.search(pattern, value_check, re.IGNORECASE):
                return True
    ...
    def validate(self, value: str, validation_level: ValidationLevel = None) -> ValidationResult:
        ...
        try:
            ...
            # Security: Check for malicious patterns before regex execution
