...
class PhoneValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, phone: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in phone input
        ...
        malicious_patterns = [
            ...
            # Non-phone characters (except expected)
            r'[^0-9\+\-\(\)\.\s]',
        ]
        
        phone_check = phone.lower()
        for pattern in malicious_patterns:
            if re.search(pattern, phone_check, re.IGNORECASE):
                return True
    ...
    def validate(self, phone: str, country: Optional[str] = None,
        ...
        try:
            ...
            MAX_SECURE_LENGTH = 50  # Reasonable max for international numbers
            if len(phone) > MAX_SECURE_LENGTH:
                errors.append(f"Phone number too long (max {MAX_SECURE_LENGTH} characters)")
                return self._create_result(False, errors, metadata, 0.0)
            
            # Security: Check for malicious patterns
            if self._contains_malicious_patterns(phone):
                errors.append("Phone number contains potentially malicious content")
                return self._create_result(False, errors, metadata, 0.0)
