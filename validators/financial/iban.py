...
class IBANValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, iban: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in IBAN input
        ...
        malicious_patterns = [
            ...
            # Non-IBAN characters (except expected alphanumeric)
            r'[^0-9A-Z\s]',
        ]
        
        iban_check = iban.upper()
        for pattern in malicious_patterns:
            if re.search(pattern, iban_check, re.IGNORECASE):
                return True
    ...
    def validate(self, 
        ...
        try:
            ...
            MAX_IBAN_LENGTH = 50  # IBAN max is 34 chars, add buffer
            if len(iban) > MAX_IBAN_LENGTH:
                return ValidationResult(
                    is_valid=False,
                    id_type=IDType.IBAN,
                    confidence=0.0,
                    errors=[f'IBAN too long (max {MAX_IBAN_LENGTH} characters)'],
                    metadata={}
                )
            ...
            if self._contains_malicious_patterns(iban):
                return ValidationResult(
                    ...
                    id_type=IDType.IBAN,
                    confidence=0.0,
                    errors=['IBAN contains potentially malicious content'],
                    metadata={}
                )
