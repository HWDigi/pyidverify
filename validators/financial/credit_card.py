...
class CreditCardValidator(BaseValidator):
    ...
    def validate(self, card_number: str, expiry: Optional[str] = None,
        ...
        try:
            ...
            if len(card_number) > 30:  # Max reasonable card length with spaces/dashes
                errors.append("Card number too long (max 30 characters for security)")
                return self._create_result(False, errors, metadata, 0.0)
            ...
            if self._contains_malicious_patterns(card_number):
    ...
    def _contains_malicious_patterns(self, card_number: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in card number input
        ...
        malicious_patterns = [
            ...
            # Non-numeric/space/dash characters (except expected)
            r'[^0-9\-\s]',
        ]
        
        card_check = card_number.lower()
        for pattern in malicious_patterns:
            if re.search(pattern, card_check, re.IGNORECASE):
                return True
