...
class SSNValidator(BaseValidator):
    ...
    def validate(self, ssn: str, validation_level: 'ValidationLevel' = None) -> 'ValidationResult':
        ...
        try:
            ...
            # SECURITY FIX #1: Enhanced input validation and sanitization
            ...
            if len(ssn) > 20:  # Max reasonable SSN length with separators
                errors.append("SSN too long (max 20 characters for security)")
                return self._create_result(False, errors, metadata, 0.0)
            ...
            if self._contains_malicious_patterns(ssn):
                errors.append("Potentially malicious input detected")
                metadata['security_flag'] = 'malicious_pattern_detected'
                return self._create_result(False, errors, metadata, 0.0)
            
            # PHASE 3 SECURITY FIX #6: Advanced input sanitization for sensitive data
            sanitization_result = self._advanced_sanitize_ssn_input(ssn)
            if not sanitization_result['is_safe']:
                errors.append(f"SSN failed advanced security screening: {sanitization_result['risk_level']}")
                metadata['sanitization_threats'] = sanitization_result['threats_detected']
                metadata['security_flag'] = 'advanced_sanitization_failed'
                # Log security event for sensitive data attempt
                self._log_security_event('ssn_sanitization_failure', sanitization_result['threats_detected'])
                return self._create_result(False, errors, metadata, 0.0)
            
            # Use sanitized input for processing
            sanitized_ssn = sanitization_result['sanitized_value']
            if sanitized_ssn != ssn:
                metadata['input_sanitized'] = True
                metadata['sanitization_applied'] = sanitization_result['sanitization_applied']
            
            # Normalize SSN
            normalized_ssn = self._normalize_ssn(sanitized_ssn)
    ...
    def _contains_malicious_patterns(self, ssn: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in SSN input
        ...
        malicious_patterns = [
            ...
            # Non-numeric/hyphen/space characters (except expected)
            r'[^0-9\-\s]',
        ]
        
        ssn_check = ssn.lower()
        for pattern in malicious_patterns:
            if re.search(pattern, ssn_check, re.IGNORECASE):
                return True
                
        return False
    
    def _advanced_sanitize_ssn_input(self, ssn: str) -> Dict[str, Any]:
        """
        PHASE 3 SECURITY FIX #6: Advanced input sanitization for SSN (sensitive data)
        
        Args:
            ssn: SSN input to sanitize and analyze
        ...
        try:
            # 1. Unicode normalization for SSN (critical for sensitive data)
            normalized = unicodedata.normalize('NFC', ssn)
            if normalized != ssn:
                sanitization_applied.append('unicode_normalization')
                sanitized_ssn = normalized
            
            # 2. Aggressive control character removal for SSN
            original_length = len(sanitized_ssn)
            sanitized_ssn = re.sub(r'[\u0000-\u001F\u007F-\u009F\uFEFF\u200B-\u200D]', '', sanitized_ssn)
            if len(sanitized_ssn) < original_length:
                threats_detected.append('control_characters')
                sanitization_applied.append('control_char_removal')
            
            # 3. Remove any encoding artifacts that could hide SSNs
            if '%' in sanitized_ssn and re.search(r'%[0-9A-Fa-f]{2}', sanitized_ssn):
                threats_detected.append('url_encoding_in_ssn')
                try:
                    from urllib.parse import unquote
                    decoded = unquote(sanitized_ssn)
                    # Only accept if it looks like a valid SSN pattern
                    if re.match(r'^[\d\s\-]*$', decoded):
                        sanitized_ssn = decoded
                        sanitization_applied.append('url_decode')
                except:
                    pass
            
            # 4. HTML entity removal (critical for SSN)
            if '&' in sanitized_ssn:
                threats_detected.append('html_entities_in_ssn')
                try:
                    decoded = html.unescape(sanitized_ssn)
                    if re.match(r'^[\d\s\-]*$', decoded):
                        sanitized_ssn = decoded
            ...
            # 5. Strict character filtering for SSN (only digits, hyphens, spaces)
            original_length = len(sanitized_ssn)
            sanitized_ssn = re.sub(r'[^0-9\-\s]', '', sanitized_ssn)
            if len(sanitized_ssn) < original_length:
                threats_detected.append('invalid_ssn_characters')
                sanitization_applied.append('character_filtering')
            
            # 6. Length validation after sanitization
            if len(sanitized_ssn.replace('-', '').replace(' ', '')) != 9:
                if len(sanitized_ssn.replace('-', '').replace(' ', '')) == 0:
                    threats_detected.append('empty_after_sanitization')
                else:
                    threats_detected.append('invalid_length_after_sanitization')
            
            # 7. Pattern validation - must look like SSN
            digits_only = sanitized_ssn.replace('-', '').replace(' ', '')
            if digits_only and not re.match(r'^\d{9}$', digits_only):
                threats_detected.append('invalid_ssn_pattern')
            
            # 8. Calculate risk level (higher threshold for SSN)
            risk_level = 'LOW'
            if len(threats_detected) > 2 or any('encoding' in threat for threat in threats_detected):
                risk_level = 'HIGH'
            elif len(threats_detected) > 0:
                risk_level = 'MEDIUM'
            
            # 9. SSN-specific safety check (more restrictive)
            is_safe = (risk_level == 'LOW' and 
                      len(sanitized_ssn.replace('-', '').replace(' ', '')) == 9 and
                      sanitized_ssn.replace('-', '').replace(' ', '').isdigit())
            
            return {
                'sanitized_value': sanitized_ssn,
                'original_value': ssn,
            ...
            # For SSN, any sanitization error is high risk
            return {
                'sanitized_value': '',
                'original_value': ssn,
    ...
    def _log_security_event(self, event_type: str, details: Any) -> None:
        """
        PHASE 3 SECURITY: Log security events for SSN validation
        
        Args:
            event_type: Type of security event
            details: Event details (will be anonymized)
        """
        try:
            # Log security event without exposing sensitive SSN data
            security_event = {
                'validator_type': 'ssn',
                'event_type': event_type,
                'timestamp': time.time(),
                'details': str(details)[:100],  # Limit detail length
                'severity': 'HIGH' if 'sanitization_failure' in event_type else 'MEDIUM'
            }
            
            # In production, this would integrate with security monitoring system
            if hasattr(self, 'security_logger'):
                self.security_logger.warning(f"SSN Security Event: {event_type}")
            
        except Exception:
            # Don't let logging errors affect validation
            pass
