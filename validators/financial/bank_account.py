...
class BankAccountValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, account_data: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in bank account input
        
        Args:
            account_data: Account or routing number to check
        ...
        malicious_patterns = [
            ...
            # Non-bank account characters (except expected)
            r'[^0-9A-Z\-\s]',
        ]
        
        account_check = account_data.upper()
        for pattern in malicious_patterns:
            if re.search(pattern, account_check, re.IGNORECASE):
                return True
    ...
    def validate_us_account(self, routing_number: str, account_number: str, 
        ...
        try:
            ...
            # SECURITY FIX #1: Enhanced input validation
            ...
            MAX_ROUTING_LENGTH = 20  # ABA is 9 digits, allow some formatting
            MAX_ACCOUNT_LENGTH = 40  # Account numbers vary, but reasonable limit
            
            if len(routing_number) > MAX_ROUTING_LENGTH:
                errors.append(f"Routing number too long (max {MAX_ROUTING_LENGTH} characters)")
                return self._create_result(False, errors, metadata, 0.0)
                
            if len(account_number) > MAX_ACCOUNT_LENGTH:
                errors.append(f"Account number too long (max {MAX_ACCOUNT_LENGTH} characters)")
                return self._create_result(False, errors, metadata, 0.0)
            
            # Security: Check for malicious patterns
            if self._contains_malicious_patterns(routing_number) or self._contains_malicious_patterns(account_number):
                errors.append("Bank account details contain potentially malicious content")
                return self._create_result(False, errors, metadata, 0.0)
    ...
    def validate_iban(self, iban: str, validation_level: 'ValidationLevel' = None) -> 'ValidationResult':
        ...
        try:
            ...
            # Security: Length validation to prevent buffer overflow attacks
            MAX_IBAN_LENGTH = 50  # IBAN max is 34 chars, allow some buffer
            if len(iban) > MAX_IBAN_LENGTH:
                errors.append(f"IBAN too long (max {MAX_IBAN_LENGTH} characters)")
                return self._create_result(False, errors, metadata, 0.0)
            
            # Security: Check for malicious patterns
            if self._contains_malicious_patterns(iban):
                errors.append("IBAN contains potentially malicious content")
                return self._create_result(False, errors, metadata, 0.0)
