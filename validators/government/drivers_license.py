...
class DriversLicenseValidator(BaseValidator):
    ...
    def _contains_malicious_patterns(self, license_number: str) -> bool:
        """
        SECURITY FIX #1: Detect potentially malicious patterns in license input
        ...
        malicious_patterns = [
            ...
            # Non-license characters (except expected alphanumeric)
            r'[^0-9A-Z\-\*]',
        ]
        
        license_check = license_number.upper()
        for pattern in malicious_patterns:
            if re.search(pattern, license_check, re.IGNORECASE):
                return True
        ...
        try:
            ...
            if not isinstance(license_number, str):
                return ValidationResult(
                    ...
                    confidence=0.0,
                    errors=['License number must be a string'],
                    metadata={}
                )
            
            # Security: Length validation to prevent buffer overflow attacks
            MAX_LICENSE_LENGTH = 25  # Most DL numbers are under 20 chars
            if len(license_number) > MAX_LICENSE_LENGTH:
                return ValidationResult(
                    is_valid=False,
                    id_type=IDType.DRIVERS_LICENSE,
                    confidence=0.0,
                    errors=[f'License number too long (max {MAX_LICENSE_LENGTH} characters)'],
                    metadata={}
                )
            
            # Security: Check for malicious patterns
            if self._contains_malicious_patterns(license_number):
                return ValidationResult(
                    is_valid=False,
                    id_type=IDType.DRIVERS_LICENSE,
                    confidence=0.0,
                    errors=['License number contains potentially malicious content'],
                    metadata={}
                )
